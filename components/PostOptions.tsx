...
function PostOptions({ post }: { post: IPostDocument }) {
    ...
    const likeOrUnlikePost = async () => {
        ...
        const newLikes = liked ? likes?.filter((like) => 
            like !== user.id) : [...(likes || []), user.id];
    ...
    <div>
        ...
        <div className="flex p-2 justify-between px-2 border">
            <Button
                variant="ghost"
                className="postButton"
                // Onclick handler to like or unlike the post
                onClick={() => {
